# DeepSeekToolkit 竞品差距收敛 — PRD

**日期:** 2026-05-10
**来源:** DeepSeekToolkit vs LangChain vs CrewAI 三方benchmark

---

## 背景

2026年5月完成了一轮完整的三方agent框架对比benchmark：使用相同的DeepSeek API（deepseek-v4-pro），相同的4个agent类型（财务分析/投资分析/数据分析/影视策划），相同的4个工具（read_file/web_search/calculate/save_result），对比DeepSeekToolkit、LangChain 1.2.18、CrewAI 1.14.4的表现。

### Benchmark 关键数据

```
                    DeepSeekToolkit    LangChain        CrewAI
...
saver.save()  # -> output/runtime_dumps/DeepSeekToolkit/{agent_type}/
```

保存到 `output/runtime_dumps/DeepSeekToolkit/{agent_type}/` 的三件套：
...
```python
# 搜索后端注册
from deepseek_toolkit.tools import SearchProvider, DuckDuckGoProvider, BingProvider
...
| `AgentRuntimeSaver` | 新增 | `deepseek_toolkit_agent.py`，benchmark专用 |
...
1. 运行完整的12 agent benchmark，验证：
   ...
   - `output/runtime_dumps/DeepSeekToolkit/`下有4个agent类型×3个JSON文件 = 12个文件
   - 所有agent成功完成，0错误
...
这次benchmark验证了一个核心判断：**DeepSeekToolkit的价值不在于"比LangChain做得更多"，而在于"LangChain做不了的事"。**
